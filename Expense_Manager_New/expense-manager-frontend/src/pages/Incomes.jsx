...
const Incomes = () => {
    ...
    const incomeCategories = (data?.categories || []).filter(c => c && c.IsIncome == 1);
    const selectedCategory = incomeCategories.find(c => c.CategoryID === parseInt(form.category));
    ...
    const handleSubmit = async (e) => {
        ...
        let success = false;
        if (isEditing) {
            success = await updateItem('incomes', form.id, payload);
        } else {
            success = await addItem('incomes', payload);
        }
        setLoading(false);
        if (success) {
            resetForm();
    ...
    const handleEdit = (item) => {
        setForm({
            ...item,
            id: item.IncomeID,
            date: item.IncomeDate ? new Date(item.IncomeDate).toISOString().split('T')[0] : '',
        ...
        <div className="dashboard-container">
            ...
            <div className="dashboard-header" style={{ marginBottom: '2rem' }}>
                <div className="dashboard-title">
                    <h1 style={{ fontSize: '1.75rem', fontWeight: 700 }}>Income Management</h1>
                    <p style={{ color: 'var(--text-muted)', fontSize: '0.9rem' }}>Track and manage your incoming revenue</p>
                </div>
                <div className="dashboard-actions">
                    ...
                    >
                        {showForm ? 'Close Form' : <><Plus size={18} style={{ marginRight: '8px' }} /> Add Income</>}
                    </button>
                </div>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
                <div style={{ background: 'rgba(255,255,255,0.05)', padding: '1.25rem', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
                    <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: '0.5rem' }}>Total Monthly Income</p>
                    <h3 style={{ fontSize: '1.5rem', fontWeight: 700, margin: 0 }}>₹{(data?.incomes || []).reduce((sum, e) => sum + Number(e.Amount || 0), 0).toLocaleString()}</h3>
                </div>
                <div style={{ background: 'rgba(255,255,255,0.05)', padding: '1.25rem', borderRadius: '16px', border: '1px solid var(--border-color)' }}>
                    ...
                    <h3 style={{ fontSize: '1.5rem', fontWeight: 700, margin: 0 }}>{(data?.incomes || []).length}</h3>
                </div>
            </div>
                    ...
                    <Card title="Income Logs">
                        <div className="table-responsive">
                            <table className="table">
                                ...
                                <tbody>
                                    {(data?.incomes || []).filter(item => item).map(item => (
                                        <tr key={item.IncomeID}>
                                            <td>
                                                <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                                                    <span style={{ fontWeight: 600 }}>{getCategoryName(item.CategoryID)}</span>
                                                    <span style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                                                        {getSubCategoryName(item.CategoryID, item.SubCategoryID)} • {item.IncomeDate ? item.IncomeDate.split('T')[0].split('-').reverse().join('-') : ''}
                                                    </span>
                                                    {item.Description && (
                                            ...
                                            <td style={{ textAlign: 'right' }}>
                                                <div style={{ display: 'flex', gap: '0.5rem', justifyContent: 'flex-end' }}>
                                                    ...
                                                    <button
                                                        onClick={() => { if (window.confirm('Delete income?')) deleteItem('incomes', item.IncomeID) }}
                                                        className="btn-icon"
